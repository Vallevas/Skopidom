...
async function deploy() {
    ...
    const contractPath = path.join(__dirname, '..', 'build', 'InventoryAudit.bin');
    const abiPath = path.join(__dirname, '..', 'build', 'InventoryAudit.abi');
    
    if (!fs.existsSync(contractPath)) {
        console.error('Contract bytecode not found. Please compile first: npm run compile');
        console.log('Expected path:', contractPath);
        process.exit(1);
    }
    
    const bytecode = fs.readFileSync(contractPath, 'utf8').trim();
    ...
    console.log(`BLOCKCHAIN_CONTRACT_ADDRESS=${deployedContract.options.address}`);
